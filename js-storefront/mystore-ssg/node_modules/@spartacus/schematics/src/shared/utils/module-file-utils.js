...
function addImport(host, filePath, importText, importPath) {
    const moduleSource = (0, file_utils_1.getTsSourceFile)(host, filePath);
    if (!(0, ast_utils_1.isImported)(moduleSource, importText, importPath)) {
        const change = (0, ast_utils_1.insertImport)(moduleSource, filePath, importText, importPath);
...
function createImportChange(host, filePath, importText, importPath) {
    const source = (0, file_utils_1.getTsSourceFile)(host, filePath);
...
function getTemplateUrlOrInlineTemplate(source, templateOrTemplateUrl) {
    const decorator = (0, ast_utils_1.getDecoratorMetadata)(source, 'Component', constants_1.ANGULAR_CORE)[0];
    if (!decorator) {
        return undefined;
    }
    const templateMetadata = (0, file_utils_1.getMetadataProperty)(decorator, templateOrTemplateUrl);
    if (!templateMetadata) {
        return undefined;
    }
    let stringNode;
...
function getAppModule(host, project) {
    const projectTargets = (0, workspace_utils_1.getProjectTargets)(host, project);
